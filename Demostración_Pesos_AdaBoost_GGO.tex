...
\documentclass[11pt]{article}
    ...
    % Document parameters
    % Document title
    \title{Demostraci?n\_Pesos\_AdaBoost\_GGO}
...
\[w_i^{(m+1)} = w_i^{(m)}\cdot \exp(\alpha_m\cdot\cdot\mathbb{1}\left(y_i \neq G_m(x_i)\right)\cdot \exp(\beta_m)\]
con \(\alpha_m = 2\beta_m\)

Para lo anterior, usaremos el método de \emph{Forward Stagewise Additive
Modeling (FSAM)} por sus siglas en inglés, para encontrar los valores
óptimos para la función de costo exponencial
\[L(y,f(x)) = \exp(-yf(x))\]\\
además, es importante recordar que para el modelo de \textbf{AdaBoost},
las funciones base (\emph{basis}), están dadas por los clasificadores
individuales \(G_m(x) \in \lbrace -1,1 \rbrace\). Teniendo todo lo
anterior en consideración, se busca resolver el siguiente problema de
minimización
...
Primero expandimos el producto y utilizamos las propiedades de la
función exponencial para abrir la suma de sus exponentes en el producto
de dos exponenciales, por lo cual, se tiene la siguiente expresión de
\eqref{f_opt}
...
Sea
\(w_i^{(m)} = \exp\left(-y_i\cdot f_{m-1}(X_i)\right)\label{eq:pesos}\tag{3}\)

Notemos que \[-y_i\beta G(x_i) = \left\{ \begin{array}{cc}
         \beta & \mbox{sí $y_i \neq G(x_i)$}\\
        -\beta & \mbox{sí $y_i = G(x_i)$}\end{array} \right.\]

Por lo cual, \eqref{f_opt_2}, lo podemos expresar en dos sumas, una que
corrar sobre todos los índices \(i\), para los cuales se cumpla que
\(y_i = G(x_i)\), y la otra que corra sobre los índices \(i\) tales que
\(y_i \neq G(x_i)\), por lo cual, se tiene que el argumento que queremos
minimizar en \eqref{f_opt_2}, es igual a
...
Notemos que \eqref{eq:aux} lo podemos expresar como
\[\left(\exp(\beta)-\exp(-\beta)\right)\cdot \sum\limits_{i=1}^{n}w_i^{(m)}\mathbb{1}\left(y_i \neq G(x_i)\right)+\exp(-\beta)\sum\limits_{i=1}^{n}w_i^{(m)}\]
pues como
...
tenemos que
\(\exp(\beta)\sum\limits_{i=1}^{n}w_i^{(m)}\mathbb{1}\left(y_i \neq G(x_i)\right) = \exp(\beta)\cdot\sum\limits_{y_i \neq G(x_i)}w_i^{(m)}\)
(primer elemento de \eqref{eq:aux})\\
y por otro lado
\(\exp(-\beta)\sum\limits_{y_i = G(x_i)}w_i^{(m)} = \exp(-\beta)\sum\limits_{i=1}^{n}w_i^{(m)} - \exp(-\beta)\sum\limits_{i=1}^{n}w_i^{(m)}\mathbb{1}\left(y_i \neq G(x_i)\right)\)
(segundo elemento de \eqref{eq:aux}), es decir, le estamos quitando
todas las \(w_i^{(m)}\) tales que \(y_i \neq G(x_i)\)

Por lo tanto, podemos expresar a \eqref{f_opt} como \begin{equation}
...
Por lo tanto, notemos que para minimizar la expresión anterior para
\(G\), \(\forall \beta >0\), se tiene que que la \(G\), que minimiza
\eqref{eq:opt_f}, es
\[G_m = \underset{G}{argmin}\sum\limits_{i=1}^{n}w_i^{(m)}\mathbb{1}\left(y_i \neq G(x_i)\right)\]
pues \(\left(\exp(\beta)-\exp(-\beta)\right)\) es una constante, así
como \(\exp(-\beta)\sum\limits_{i=1}^{n}w_i^{(m)}\).

Por otro lado, para minimizar \eqref{eq:opt_f} con respecto a \(\beta\),
obtenemos la derivada del argumento de \eqref{eq:opt_f}, con respecto a
\(\beta\), se tiene que \begin{align*}
...
Por lo tanto, se tiene que los valores de \(\beta\) y \(G\), que
minimizan \eqref{eq:opt_f} son
...
Entonces, como \(f_m(x) = f_{m-1}(x) + \beta_mG_m(x)\) \begin{align*}
...
\(-y_iG_m(x_i) = 2\cdot\mathbb{1}\left(y_i \neq G_m(x_i)\right) - 1\),
pues
